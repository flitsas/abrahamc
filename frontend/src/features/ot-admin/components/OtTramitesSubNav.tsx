import type { OtTramitesSubNavItem } from "../lib/otTramitesSubNav.js";

type OtTramitesSubNavProps = {
  items: OtTramitesSubNavItem[];
  activeId: string;
  onChange: (id: string) => void;
};

export function OtTramitesSubNav({
  items,
  activeId,
  onChange,
}: OtTramitesSubNavProps) {
  const activeItem = items.find((item) => item.id === activeId);

  return (
    <div className="space-y-4">
      <nav
        aria-label="Secciones de consola OT"
