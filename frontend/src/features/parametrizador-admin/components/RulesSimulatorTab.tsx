...
import {
  useCanSimulateRules,
} from "../hooks/useCanManageParametrizacion.js";
...
export function RulesSimulatorTab({
  ...
  if (matrixQuery.isError) {
    return (
      <ErrorState error={matrixQuery.error} onRetry={() => matrixQuery.refetch()} />
    );
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-flit-draft/10">
                {rules.map((rule) => (
                  <tr key={rule.id}>
                    <td className="px-3 py-2 text-flit-blueDark">{rule.name}</td>
                    <td className="px-3 py-2 text-flit-muted">{rule.priority}</td>
      ...
      {canSimulate ? (
        ...
        >
          <h2 id="simulator-heading" className="text-base font-bold text-flit-blueText">
            Simulador de reglas
          </h2>
          <form className="mt-4 space-y-3" onSubmit={handleSimulate} noValidate>
            <div>
              ...
              <textarea
                ...
                aria-describedby={
                  jsonError || fieldErrors.tenantId ? "rules-payload-error" : undefined
                }
              />
...
type SimulationResultsProps = {
  simulation: NonNullable<
    ReturnType<typeof useSimulateRules>["data"]
  >;
};
