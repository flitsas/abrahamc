...
| `backend/dotnet/src/` | Backend |
| `backend/dotnet/tests/` o `frontend/src/**/*.spec.*` | Test existente — NO sobreescribir |
...
**Proyecto:** `backend/dotnet/tests/<FLIT>.<Modulo>.Tests/`
Si no existe, crearlo:

```bash
cd backend/dotnet
dotnet new xunit -n FLIT.<Modulo>.Tests -o tests/FLIT.<Modulo>.Tests
dotnet sln add tests/FLIT.<Modulo>.Tests/FLIT.<Modulo>.Tests.csproj
...
```bash
cd backend/dotnet
dotnet test tests/<FLIT>.<Modulo>.Tests/ --logger "console;verbosity=detailed" \
...
dotnet test backend/dotnet/ --filter "FullyQualifiedName~<Modulo>"
```
